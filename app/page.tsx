    ...
    <div className="flex h-screen w-full bg-[#030305] text-slate-200 overflow-hidden font-sans">
      <aside className="w-72 bg-[#0A0A0F] border-r border-white/5 flex flex-col p-6">
        ...
        <nav className="flex-1 space-y-2">
          <button onClick={() => setCurrentView('chat')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-[10px] font-bold uppercase italic ${currentView === 'chat' ? 'bg-white/10 text-white' : 'text-slate-500'}`}><MessageSquare size={16}/> Intelligence Chat</button>
          <button onClick={() => setCurrentView('admin')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-[10px] font-bold uppercase italic ${currentView === 'admin' ? 'bg-white/10 text-white' : 'text-slate-500'}`}><ShieldCheck size={16}/> Admin Console</button>
        </nav>
