...
// Logic: If this file is in 'app/page.tsx', it looks into the 'landing' folder
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#0A0A0F] border-r border-white/5 transform transition-transform lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full p-6">
          <div className="flex items-center gap-3 mb-10 px-2">
            ...
            <h1 className="text-xl font-black italic uppercase tracking-tighter text-white tracking-widest">NEXORA AI</h1>
          </div>

          <nav className="flex-1 space-y-2">
            <button 
              onClick={() => setCurrentView('chat')} 
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest italic transition-all ${currentView === 'chat' ? 'bg-white/10 text-white' : 'text-slate-500 hover:text-white'}`}
            >
            ...
            {isAdmin && (
              <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest italic transition-all ${currentView === 'admin' ? 'bg-white/10 text-white' : 'text-slate-500 hover:text-white'}`}
              >
          ...
          <button 
            ...
            className="mt-auto flex items-center gap-3 px-4 py-3 text-slate-600 hover:text-red-400 text-[10px] font-black uppercase tracking-[0.2em] transition-colors"
          >
                            ...
                            code({inline, className, children}: any) {
                              ...
                              return !inline && match ? (
                                <SyntaxHighlighter 
                                  ...
                                  language={match[1]} 
                                  PreTag="div" 
                                  customStyle={{borderRadius: '1rem', background: '#0D0D12', border: '1px solid rgba(255,255,255,0.05)'}}
                                >
                                  {String(children)}
                                </SyntaxHighlighter>
              ...
              <form onSubmit={handleSubmit} className="p-6 lg:p-12 max-w-4xl mx-auto w-full sticky bottom-0 bg-gradient-to-t from-[#030305] via-[#030305] to-transparent">
                <div className="relative flex items-center bg-[#0D0D14] border border-white/10 p-2.5 rounded-[2.5rem] shadow-2xl focus-within:border-[#6C63FF]/50 transition-all group">
                  <input 
                    ...
                    placeholder="Command Nexora Intelligence..." 
                    className="flex-1 bg-transparent border-none outline-none px-6 text-white text-sm placeholder:text-slate-600" 
                  />
                  <button 
                    type="submit" 
                    disabled={isLoading || !input?.trim()} 
                    className="w-14 h-14 bg-[#6C63FF] rounded-3xl flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-xl disabled:opacity-50 disabled:grayscale"
                  >
