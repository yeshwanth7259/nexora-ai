      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#0A0A0F] border-r border-white/5 transform transition-transform lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full p-6">
          ...
          <nav className="flex-1 space-y-2">
            <button onClick={() => setCurrentView('chat')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest italic transition-all ${currentView === 'chat' ? 'bg-white/10 text-white border border-white/10' : 'text-slate-500 hover:text-white'}`}>
              <MessageSquare size={16}/> Intelligence Chat
            </button>
            {isAdmin && (
              <button onClick={() => setCurrentView('admin')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest italic transition-all ${currentView === 'admin' ? 'bg-white/10 text-white border border-white/10' : 'text-slate-500 hover:text-white'}`}>
          ...
          {currentView === 'admin' ? (
             <div className="p-8 lg:p-12 animate-in fade-in duration-500">
                ...
                <div className="bg-[#0A0A0F] border border-white/5 p-8 rounded-[2.5rem] w-fit min-w-[300px] shadow-2xl">
                   ...
                   <p className="text-slate-500 text-[10px] font-black uppercase mb-1 tracking-widest text-white">Authorized Access</p>
                </div>
             </div>
          ) : (
            <div className="flex flex-col h-full">
                ...
                <div className="max-w-4xl mx-auto">
                  {messages.length === 0 ? (
                    <div className="py-20 text-center opacity-20">
                      <h2 className="text-xs font-black text-white tracking-[0.8em] uppercase italic">Nexora Intelligence // Build The Future</h2>
                    </div>
                  ) : (
              ...
              <form onSubmit={handleSubmit} className="p-6 lg:p-12 max-w-4xl mx-auto w-full sticky bottom-0 bg-gradient-to-t from-[#030305] via-[#030305] to-transparent">
                <div className="relative flex items-center bg-[#0D0D14] border border-white/10 p-2.5 rounded-[2.5rem] shadow-2xl focus-within:border-[#6C63FF]/40 transition-all">
                  ...
                  <button type="submit" disabled={isLoading || !input?.trim()} className="w-14 h-14 bg-[#6C63FF] rounded-3xl flex items-center justify-center shadow-xl hover:scale-105 active:scale-95 transition-all">
